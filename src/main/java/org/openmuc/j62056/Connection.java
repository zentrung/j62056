...
public class Connection {
     ...
     *
     * @param serialPort          examples for serial port identifiers are on Linux "/dev/ttyS0" or "/dev/ttyUSB0" and on Windows "COM1"
     * @param handleEcho          tells the connection to throw away echos of outgoing messages. Echos are caused by some optical
     *                            transceivers.
     * @param baudRateChangeDelay tells the connection the time in ms to wait before changing the baud rate during message exchange.
    ...
    /**
     * Creates a Connection object. The option handleEcho is set to false and the baudRateChangeDelay is set to 0.
     *
     * @param serialPort examples for serial port identifiers on Linux are "/dev/ttyS0" or "/dev/ttyUSB0" and on Windows "COM1"
     */
     ...
     * @throws IOException      if any kind of error other than timeout occurs while trying to read the remote device. Note that the
     *                          connection is not closed when an IOException is thrown.
     * @throws TimeoutException if no response at all (not even a single byte) was received from the meter within the timeout span.
     */
    public List<DataSet> read() throws IOException, TimeoutException {
        ...
        // System.out
        // .println("Got the following identification message: " + getByteArrayString(buffer, numBytesReadTotal));
